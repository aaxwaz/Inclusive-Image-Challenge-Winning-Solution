...
def get_classes_for_images_dict_top_543():
    cache_file = OUTPUT_PATH + 'overall_classes_dict_top_543.pklz'

    if not os.path.isfile(cache_file):
        index_arr_forward, index_arr_backward = get_classes_to_index_dicts_top_543()
        d1, d2 = get_classes_dict_top_543('single')
        res = dict()
        limit = 1000000000000
...
def get_classes_for_images_dict_top_491_from_test():
    cache_file = OUTPUT_PATH + 'overall_classes_dict_top_491_from_test.pklz'

    if not os.path.isfile(cache_file):
        index_arr_forward, index_arr_backward = get_classes_to_index_dicts_top_491()
        d1, d2 = get_classes_dict_top_491('single')
        res = dict()
        limit = 1000000000000
...
def get_classes_for_tst_images_dict_top_491():
    s1 = pd.read_csv(INPUT_PATH + 'tuning_labels.csv')
    index_arr_forward, index_arr_backward = get_classes_to_index_dicts_top_491()
    d1, d2 = get_classes_dict('single')
    res = dict()
...
def get_tuning_labels_data_for_validation_top_491(box_size=299):
    cache_path = OUTPUT_PATH + 'tuning_labels_data_{}_top_491.pklz'.format(box_size)
    if not os.path.isfile(cache_path):
        s1 = pd.read_csv(INPUT_PATH + 'tuning_labels.csv')
        test_image_classes = get_classes_for_tst_images_dict_top_491()
        ids = list(s1['image_id'].values)
        images = []
        images_target = get_target_top_491(ids, test_image_classes).astype(np.uint8)
        for i, id in enumerate(ids):
            path =  INPUT_PATH + 'stage_1_test_images/' + id + '.jpg'
            img = read_single_image(path)
            img = cv2.resize(img, (box_size, box_size), cv2.INTER_LANCZOS4)
            images.append(img)
        images = np.array(images, dtype=np.float32)
        images = inception_resnet_preprocess_input(images)
        save_in_file((images, images_target), cache_path)
    else:
        images, images_target = load_from_file(cache_path)
...
'''
/m/01g317 /m/09j2d /m/0dzct /m/07j7r /m/05s2s LB: 0.254    
'''